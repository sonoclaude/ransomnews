...
ITALY_KEYWORDS = [
    ...
    r"\.it\b",
    # Forme societarie standard
    r"\bS\.r\.l\b", r"\bSrl\b", r"\bS\.p\.A\b", r"\bSpA\b",
    r"\bs\.n\.c\b", r"\bsnc\b", r"\bs\.a\.s\b", r"\bsas\b",
    # Forme societarie aggiuntive (inequivocabilmente italiane)
    r"\bS\.r\.l\.s\b", r"\bSrls\b",
    r"\bS\.c\.r\.l\b", r"\bScrl\b",
    r"\bS\.c\.a\.r\.l\b", r"\bScarl\b",
    r"\bS\.c\.s\b", r"\bScs\b",
    r"\bS\.c\.p\.a\b", r"\bScpa\b",
    # Enti del terzo settore / no-profit
    r"\bOnlus\b", r"\bETS\b", r"\bAPS\b", r"\bASD\b",
    # Città principali
    "milano", "roma", "napoli", "torino", "bologna",
    "firenze", "venezia", "genova", "palermo", "bari",
    "brescia", "padova", "catania", "trieste", "verona",
    "messina", "taranto", "modena", "perugia", "livorno",
    "cagliari", "prato", "parma", "reggio emilia", "reggio calabria",
    "foggia", "salerno", "ferrara", "sassari", "monza",
    # Indicatori istituzionali / PA
    "comune di", "provincia di", "regione ",
    r"\bASL\b", r"\bINPS\b", r"\bINAIL\b",
    "ospedale", "policlinico",
    r"università degli studi",
    "ministero", "prefettura", "questura",
    "camera di commercio",
]
...
ITALY_STRICT_RE = re.compile(
    ...
    r'|\bS\.r\.l\.s\b|\bSrls\b'
    r'|\bS\.c\.r\.l\b|\bScrl\b'
    r'|\bS\.c\.a\.r\.l\b|\bScarl\b'
    r'|\bcomune di\b|\bprovincia di\b'
    r'|\bASL\b|\bINPS\b|\bINAIL\b'
    r'|\bmilano\b|\broma\b|\bnapoli\b|\btorino\b|\bbologna\b'
    r'|\bfirenze\b|\bvenezia\b|\bgenova\b|\bpalermo\b|\bbari\b'
    r'|\bbrescia\b|\bpadova\b|\bcatania\b|\btrieste\b|\bverona\b'
    r'|\bmessina\b|\btaranto\b|\bmodena\b|\bperugia\b|\blivorno\b'
    r'|\bcagliari\b|\bparma\b)',
    re.IGNORECASE
)
...
def fetch_ecrime_bsky(seen: set) -> list:
    ...
    try:
        ...
        for item in r.json().get("feed", []):
            ...
            # Escludi se Location indica esplicitamente un paese non-italiano
            ITALY_LOCS = {"italy", "italia", "it", "italian"}
            loc_tags   = re.findall(r'#(\w+)', parsed["location"])
            if loc_tags and not any(t.lower() in ITALY_LOCS for t in loc_tags):
                continue
